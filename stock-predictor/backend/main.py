...
async def predict(ticker: str = "AAPL", days: int = 7):
    ...
    try:
        ...
        if hist.empty or len(hist) < 60:
            raise HTTPException(status_code=404, detail="Not enough historical data found for ticker")
        ...
        if os.path.exists(model_path) and os.path.exists(scaler_path):
            try:
                ...
                current_batch = recent_60_days.reshape(-1, 1)
                ...
                for i in range(1, days + 1):
                    ...
                    input_seq = np.append(input_seq[:, 1:, :], [next_scaled_price], axis=1)
                    
            except Exception as ml_err:
                print(f"[LSTM Load Warning] {ml_err} - Falling back to math simulation.")
                ml_active = False
        ...
        if not ml_active:
            ...
            for i in range(1, days + 1):
                future_date = hist.index[-1] + timedelta(days=i)
                labels.append(future_date.strftime("%b %d"))
                simulated_val = current_price * (1 + (random.random() - 0.45) * volatility)
